...
def export_gene_activity_to_loom(gene_activity_matrix: Union['CistopicImputedFeatures', pd.DataFrame],
    ...
    **kwargs
    ...
    if cluster_annotation is not None:
        for annotation in cluster_annotation:
            if annotation not in cell_data:
                log.error('The cluster annotation', annotation, ' is not included in cistopic_obj.cell_data')
                
    # Prepare data for minimum loom file
    ## Matrix
    if not isinstance(gene_activity_matrix, pd.DataFrame):
        if isinstance(gene_activity_matrix.mtx, sparse.csr_matrix):
            ex_mtx = pd.DataFrame.sparse.from_spmatrix(gene_activity_matrix.mtx.T, index=cell_names, columns=gene_names).sparse.to_dense()
        else:
            ex_mtx = pd.DataFrame(gene_activity_matrix.mtx.T, index=cell_names, columns=gene_names)
    else:
    ...
    if cluster_markers is not None:
        ...
        annotation_in_markers = [x for x in cluster_annotation if x in cluster_markers.keys()]
        annotation_not_in_markers = [x for x in cluster_annotation if x not in cluster_markers.keys()]
        for x in annotation_not_in_markers:
            log.info(x, 'is not in the cluster markers dictionary')
        cluster_markers = {k: v for k, v in cluster_markers.items() if k in annotation_in_markers}
        ## Keeep genes in data
        for y in cluster_markers:
            cluster_markers[y] = {x: cluster_markers[y][x][cluster_markers[y][x].index.isin(gene_names)] for x in cluster_markers[y].keys()}
    ...
    **kwargs
    ...
    if not isinstance(accessibility_matrix, pd.DataFrame):
        if isinstance(accessibility_matrix.mtx, sparse.csr_matrix):
            ex_mtx = pd.DataFrame.sparse.from_spmatrix(accessibility_matrix.mtx.T, index=cell_names, columns=region_names).sparse.to_dense()
        else:
            ex_mtx = pd.DataFrame(accessibility_matrix.mtx.T, index=cell_names, columns=region_names)
    else:
    ...
    binarized_topic_region = {x: binarized_topic_region[x][binarized_topic_region[x].index.isin(region_names)] for x in binarized_topic_region.keys()}
    topics = [Regulon(name=x, gene2weight=binarized_topic_region[x].to_dict()[x], transcription_factor=x, gene2occurrence=[]) for x in binarized_topic_region.keys()]
    ## Cell annotations and metrics
    metrics = []
    annotations = []
    for var in cell_data:
    ...
    if cluster_markers is not None:
        ...
        for y in cluster_markers:
            cluster_markers[y] = {x: cluster_markers[y][x][cluster_markers[y][x].index.isin(region_names)] for x in cluster_markers[y].keys()}
        add_markers(loom, cluster_markers)
...
def add_annotation(loom, annots: pd.DataFrame):
    ...
    for annot in annots:
        ...
        md_annot.append({"name": annot, "values": list(map(lambda x: str(x), uniq_vals.tolist()))})
...
def add_clusterings(loom: SCopeLoom,
    ...
    for cluster_name in cluster_data.columns:
        ...
        cluster_2_number = {np.unique(cluster_data[cluster_name])[i]: i for i in range(num_clusters)}

        # Data
        clusterings[str(clustering_id)] = [cluster_2_number[x] for x in cluster_data[cluster_name].tolist()]
        ...
        "ClusterID": clusterings[str(clustering_id)].values,  # Pick the first one as default clustering (this is purely arbitrary)
...
def add_markers(loom: SCopeLoom,
    ...
    for cluster_name in markers_dict:
        ...
        row_attrs_cluster_markers = {
            f"ClusterMarkers_{str(idx)}": df_to_named_matrix(cluster_markers.astype(np.int8)),
...
def get_regulons(loom):
    ...
    """
    regulon_dict = pd.DataFrame(loom.get_regulons(), index=loom.row_attrs['Gene']).to_dict()
    for t in regulon_dict:
        regulon_dict[t] = {x:y for x,y in regulon_dict[t].items() if y!=0}
    motif_data = {x['regulon']: x['motifData'] for x in loom.global_attrs['MetaData']['regulonThresholds']}
    regulon_list = [Regulon(name=x, gene2weight=regulon_dict[x], transcription_factor=x.split('_')[0], gene2occurrence=[], context=frozenset(list(motif_data[x]))) for x in regulon_dict.keys()]
    return regulon_list
