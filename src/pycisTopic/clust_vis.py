...
def find_clusters(
    ...
    Parameters
    ----------
    selected_model: `class::CistopicLDAModel`
            A cisTopic model with `class::CistopicLDAModel.cell_topic` and `class::CistopicLDAModel.topic_region` probabilities.
    target: str
    ...
    for C in res:
        ...
        clusters = pd.concat([clusters,cluster], axis=1)
    return clusters
...
def plot_metadata(
    ...
    cell_or_region_data: pd.DataFrame,
    variables: list[str],
    target: str = "cell",
    ...
    ----------
    embedding: pd.DataFrame
            The dimensionality reduction (UMAP, tSNE, etc.) coordinates.
    cell_or_region_data: pd.DataFrame
            Annotations of cells/regions to be plotted.
    variables: list[str]
            List of variables to plot. They should be included in the cell_or_region_data dataframe.
    target: str
            Whether cells ('cell') or regions ('region') should be used. Default: 'cell'
    remove_nan: bool
    ...
    data_mat = cell_or_region_data.copy()
...
def harmony(
    ...
    **kwargs,
    ...
    selected_model: `class::CistopicLDAModel`
            A cisTopic model with `class::CistopicLDAModel.cell_topic` and `class::CistopicLDAModel.topic_region` probabilities.
    cell_data: pd.DataFrame
            Annotations of cells to be used for integration.
    vars_use: list[str]
    ...
    **kwargs
    ...
    selected_model.cell_topic_harmony = cell_topic_harmony