...
class cisTopicObject:
    ...
    def __init__(self,
               ...
               cell_data: pd.DataFrame,
               region_data: pd.DataFrame,
               path_to_fragments: Union[str, List[str]],
    ...
    def merge(self,
        ...
        path_to_fragments_list = sum([x.path_to_fragments for x in cisTopic_obj_list], [])
        ...
        for i in range(1,len(region_names_list)):
            ...
            path_to_fragments_to_add = [path_to_fragments_list[i]]*len(cell_names_to_add)
            path_to_fragments=path_to_fragments+path_to_fragments_to_add
        ...
        cell_data['cisTopic_id'] = object_id
        cell_data['path_to_fragments'] = path_to_fragments
        ...
        if copy is True:
            cisTopic_obj=cisTopicObject(fragment_matrix, binary_matrix, cell_names, region_names, cell_data, region_data, list(set(path_to_fragments)), project)
            return cisTopic_obj
        else:
            self.fragment_matrix = fragment_matrix
            self.binary_matrix = binary_matrix
...
def createcisTopicObject(fragment_matrix: Union[pd.DataFrame, sparse.csr_matrix],
                         ...
                         is_acc: Optional[int] = 1,
                         path_to_fragments: Optional[str] = None,
                         project: Optional[str] = 'cisTopic'):
    """
    ...
    is_acc: int, optional
        Minimal number of fragments for a region to be considered accessible. Default: 1
    path_to_fragments: list, optional
    ...
    cell_data = pd.DataFrame([cisTopic_nr_frag, np.log10(cisTopic_nr_frag), cisTopic_nr_acc, np.log10(cisTopic_nr_acc)], columns=cell_names, index=['cisTopic_nr_frag', 'cisTopic_log_nr_frag', 'cisTopic_nr_acc', 'cisTopic_log_nr_acc']).transpose()
