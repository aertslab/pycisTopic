...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt pycisTopic
...
.PHONY: format
format:
	@black -S pycisTopic tests


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black -S --check pycisTopic tests
...
.PHONY: check-types
check-types:
	@cd src; mypy -p pycisTopic --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc pycisTopic ./tests setup.py
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/pycisTopic*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./pycisTopic-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/pycisTopic-*-py3-none-any.whl
