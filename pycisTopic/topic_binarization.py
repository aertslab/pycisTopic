    ...
    for i in range(topic_dist.shape[1]):
        ...
        if plot == True:
            ...
            if num_columns == 1:
                ...
                if plot == True:
                    plt.show()
        binarized_topics['Topic' + str(i+1)] =  pd.DataFrame(topic_dist.iloc[l_norm>thr,i]).sort_values('Topic'+str(i+1), ascending=False)
    
    if target == 'region':
        cistopic_obj.selected_model.topic_ass['Regions_in_binarized_topic'] = [binarized_topics[x].shape[0] for x in binarized_topics.keys()]
    elif target == 'cell':
        cistopic_obj.selected_model.topic_ass['Cells_in_binarized_topic'] = [binarized_topics[x].shape[0] for x in binarized_topics.keys()]
