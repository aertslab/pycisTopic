...
def barcodeRankPlot(fragments: Union[str, pd.DataFrame],
    ...
    if isinstance(plot_data, pd.DataFrame):
        ...
        if isinstance(path_to_regions, str):
        	...
        	fragments=regions.join(fragments, nb_cpu=n_cpu)
        ...
        FPB_DF.columns=['Nr_frag']
        
        FPB_DF=FPB_DF.sort_values(by=['Nr_frag'],  ascending=False)
        FPB_DF['Barcode_rank'] = range(1, len(FPB_DF) + 1)

    BR = FPB_DF.loc[:,'Barcode_rank']
    NF = FPB_DF.loc[:,'Nr_frag']
...
def insertSizeDistribution(fragments: Union[str, pd.DataFrame],
    ...
    if isinstance(plot_data, pd.DataFrame):
        ...
        fragments['Width'] = abs(fragments.End-fragments.Start).astype(str)
        fragments_per_Width=cl.Counter(fragments.Width.to_list())
        fragments_per_Width=[fragments_per_Width[x] for x in set(fragments.Width.to_list())]
        FPW_DF=pd.DataFrame(fragments_per_Width)
        FPW_DF['Width']=set(fragments.Width.to_list())
        FPW_DF.columns=['Nr_frag', 'Width']
        FPW_DF['Perc_frag']=FPW_DF['Nr_frag']/FPW_DF['Nr_frag'].sum()*100
            
        FPW_DF['Width'] = FPW_DF.loc[:,'Width'].astype(int)
...
def profileTSS(fragments: Union[str, pd.DataFrame],
               ...
               plot_data: Optional[pd.DataFrame] = None,
               n_cpu: Optional[int] = 1,
               flank_window: Optional[int] = 2000,
    ...
    if isinstance(plot_data, pd.DataFrame):
        ...
        overlap_with_TSS.start_pos = -(overlap_with_TSS.Start_b+flank_window-overlap_with_TSS.Start)*overlap_with_TSS.Strand.astype(int)
        overlap_with_TSS.end_pos = -(overlap_with_TSS.Start_b+flank_window-overlap_with_TSS.End)*overlap_with_TSS.Strand.astype(int)
        overlap_with_TSS_start = overlap_with_TSS[overlap_with_TSS.start_pos <= flank_window]
        overlap_with_TSS_start = overlap_with_TSS_start[overlap_with_TSS_start.start_pos >= -flank_window]
        overlap_with_TSS_end = overlap_with_TSS[overlap_with_TSS.end_pos <= flank_window]
        overlap_with_TSS_end = overlap_with_TSS_end[overlap_with_TSS_end.end_pos >= -flank_window]
        overlap_with_TSS_start.start_pos = overlap_with_TSS_start.start_pos+flank_window
        overlap_with_TSS_end.end_pos = overlap_with_TSS_end.end_pos+flank_window
        log.info('Creating coverage matrix')
        Position=overlap_with_TSS_start.start_pos.to_list()+overlap_with_TSS_end.end_pos.to_list()
        Barcode=overlap_with_TSS_start.Name.to_list()+overlap_with_TSS_end.Name.to_list()
        Score=overlap_with_TSS_start.Score.to_list()+overlap_with_TSS_end.Score.to_list()
        cut_sites_TSS = pd.DataFrame({'Position': Position, 'Barcode': Barcode,'Score': Score})
        if remove_duplicates == True:
            log.info('Duplicate removal')
            cut_sites_TSS.Score = 1
            
        # Make sure that all positions appear in the matrix
        cut_sites_TSS_space= pd.DataFrame([list(range(2*flank_window+1)),['Space']*(2*flank_window+1), [0]*(2*flank_window+1)], index=['Position', 'Barcode', 'Score']).T
        cut_sites_TSS=pd.concat([cut_sites_TSS,cut_sites_TSS_space], axis=0)
        
        TSS_matrix = cut_sites_TSS.groupby(["Position", "Barcode"]).agg({"Score": np.sum}).unstack(level="Position").drop('Space').fillna(0).astype(dtype)
...
def FRIP(fragments: Union[str, pd.DataFrame],
    ...
    if isinstance(plot_data, pd.DataFrame):
        ...
        fragments_in_regions_per_barcode=cl.Counter(fragments_in_regions.Name.to_list())
        fragments_in_regions_per_barcode=[fragments_in_regions_per_barcode[x] for x in set(fragments_in_regions.Name.to_list())]
        FPBIR_DF=pd.DataFrame(fragments_in_regions_per_barcode)
        FPBIR_DF.index=set(fragments_in_regions.Name.to_list())
        FPBIR_DF.columns=['Total_nr_frag_in_regions']
...
def computeQCStats(fragments_dict: Dict[str, Union[str, pd.DataFrame]],
                   ...
                   stats: Optional[List[str]]=['barcodeRankPlot', 'insertSizeDistribution', 'profileTSS', 'FRIP'],
                   ...
                   n_frag: Optional[int] = None,
                   n_bc: Optional[int] = None,
                   tss_flank_window: Optional[int] = 2000,
                   tss_window: Optional[int] = 50,
    ...
    stats: list, optional
        A list with the statistics that have to be computed. Default: All ('barcodeRankPlot', 'insertSizeDistribution', 'profileTSS', 'FRIP).
...
def plotProfile(profile_data_dict: Dict[str, pd.DataFrame],
    ...
    if 'barcodeRankPlot' in profile_list:
        ...
        for i in range(len(profile_data_dict)):
            ...
            if color is None:
                plt.plot(nosel, plot_data['Nr_frag'], sel,plot_data['Nr_frag'])
            else:
                if len(color[i]) < 2:
                    plt.plot(nosel, plot_data['Nr_frag'], sel, plot_data['Nr_frag'])
                else:
                    plt.plot(nosel, plot_data['Nr_frag'], color[i][0], sel ,plot_data['Nr_frag'], color[i][1])
            plt.xscale("log")
            plt.yscale("log")
            plt.xlabel("Barcode Rank",fontsize=10)
            plt.ylabel("Number of fragments in regions",fontsize=10)
