...
def regionWeights(imputed_acc_object,
    ...
    if use_gene_boundaries == True:
        ...
        pr_annot=pr_annot[['Chromosome', 'Start', 'End', 'Strand', 'Gene', 'Gene_width', 'Gene_size_weight']].drop_duplicate_positions()
        pr_annot=pr.PyRanges(pr_annot.df.drop_duplicates(subset ="Gene", keep = "first"))
        closest_promoter_upstream=pr_annot.nearest(pr_gene_bound, overlap=False, how='upstream')
        closest_promoter_upstream=closest_promoter_upstream[['Chromosome', 'Start', 'End', 'Strand', 'Gene', 'Distance']]
        closest_promoter_downstream=pr_annot.nearest(pr_gene_bound, overlap=False, how='downstream')
        ...
        pr_annot_df = pr_annot.df
        pr_annot_df = pr_annot_df.set_index('Gene').loc[closest_promoter_upstream.df.Gene].reset_index()
        pr_annot_df['Distance_upstream'] = closest_promoter_upstream.df.Distance
        ...
        pr_annot_df = pr_annot_df.set_index('Gene').loc[closest_promoter_downstream.df.Gene].reset_index()
        pr_annot_df['Distance_downstream'] = closest_promoter_downstream.df.Distance
