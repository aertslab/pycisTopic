...
def signature_enrichment(rankings: 'CistopicImputedFeatures',
    ...
    rankings =  pd.DataFrame(rankings.mtx.transpose(), columns = rankings.feature_names, index = rankings.cell_names)
    # Take regions in input
    if enrichment_type == 'region':
        regions_in_input = pr.PyRanges(region_names_to_coordinates(rankings.columns.tolist()))
        # Get signatures
        signatures = [region_set_to_signature(signatures[key], regions_in_input, key) for key in signatures.keys()]
    if enrichment_type == 'gene':
        # Get signatures
        signatures = [gene_set_to_signature(signatures[key], key) for key in signatures.keys()]
    # Run aucell
    auc_sig = aucell4r(
              df_rnk = rankings,
...
def gene_set_to_signature(gene_set: List,
                           name: str):
    """
